...
class SequenzDialog(QDialog):
    ...
    def __init__(self, parent, sequenz: Sequenz):
        ...
        vbox_basentext = QVBoxLayout()
        hbox_labelbutton = QHBoxLayout()
        gb_basenstr.setLayout(vbox_basentext)
        self._te_sequenztext = QPlainTextEdit(sequenz.basenstr)
        btn_basenstr = QPushButton('Basen ersetzen')
        vbox_basentext.addWidget(self._te_sequenztext)
        vbox_basentext.addLayout(hbox_labelbutton)
        hbox_labelbutton.addWidget(QLabel("Markierungen gehen verloren!"))
        hbox_labelbutton.addWidget(btn_basenstr)
